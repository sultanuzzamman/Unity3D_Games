...
public class Player : MonoBehaviour
{
	...
	void Start ()
	{
		...
		if (InitGame.halfComplete == false) {
			initialPosition = transform.position;
			targetPosition = initialPosition;

		} else {
			initialPosition = new Vector3(-8.4f,1.0f,2.6f);
...
//		
		...
		isMoving = false;
		if(transform == ControllablePlayer()&&!PauseController.isPaused&&!InitGame.matchcomplete&&!InitGame.halfComplete)
		{
...
//			}
		...
		if (isMoving == false && transform == ControllablePlayer () && !HasTheBall () && GetComponent<Animation>() ["entrada"].enabled == false&&GameManager.SharedObject().IsGameReady/* && Vector3.Distance(theBall.transform.position, transform.position)>5f*/) {
			MoveTowardsTheBall ();
			//acb.SetActive(false);
		}

		if (isMoving == false && transform == ControllablePlayer () && !HasTheBall () && GetComponent<Animation>() ["entrada"].enabled == false&&!GameManager.SharedObject().IsGameReady&&PlayerPosition.PlayerTurn/* && Vector3.Distance(theBall.transform.position, transform.position)>5f*/) {
			MoveTowardsTheBall ();
			//acb.SetActive(false);
		}
